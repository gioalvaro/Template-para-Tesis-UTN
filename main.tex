...
%%      https://github.com/gioalvaro/thesis_template
%%
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\title{Modelo de Interoperabilidad Semántica entre Sistemas de Información de Ciclo de Vida de Productos para Empresas de  Manufactura}  % use \\ here otherwise you get a justified title
                                                    % note capitalization of the title (only common 
                                                    % words in lower case)
\tagline{ }                     % tag line
\author{Alvaro Luis Fraga}                          % your name
\authorID{123456M}                                  % your University Identifier
\supervisor{Dr. Horacio Pascual Leone}              % your supervisor(s) name - no . in Dr
\cosupervisor{Dra. Maria Marcela Vegetti}            % your cosupervisor(s) name - no . in Dr ** OPTIONAL ** 
                                                    % simply comment out the above line if absent
\department{Instituto de Desarrollo y Diseño \\ (INGAR-CONICET)}  % your department (e.g. Artifical Intelligence)
\faculty{Facultad Regional Santa Fe}              % your faculty (e.g. ICT)
\degree{Doctor en Ingeniería}                      % the degree you are reading
                                                    % note the \ after the dot, so not to consider it a fullstop
\doctype{tesis}                              % the type of document (fyp, dissertation, thesis)
\degreedate{ }                        % when did you submit -- officially after your corrections !
